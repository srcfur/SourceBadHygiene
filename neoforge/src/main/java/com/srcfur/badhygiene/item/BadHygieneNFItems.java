...
public class BadHygieneNFItems {
    ...
    public static final DeferredRegister<Item> ITEMS = DeferredRegister.createItems(BadHygieneConstants.MOD_ID);
    ...
    private static Item register(String name, RegisterEvent.RegisterHelper<Item> registry, Function<Item.Properties, Item> supplier, Item.Properties baseproperties){
        Item item = supplier.apply(baseproperties.setId(ResourceKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath(BadHygieneConstants.MOD_ID, name))));
        registry.register(Identifier.fromNamespaceAndPath(BadHygieneConstants.MOD_ID, name), item);
        return item;
    }
